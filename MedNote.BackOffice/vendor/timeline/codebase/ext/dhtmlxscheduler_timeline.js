...
scheduler.render_timeline_event = function(ev, attach){
	...
	var html = '<div event_id="' + ev.id + '" title="' + ev.tooltip + '" class="' + cs + '" style="' + bg_color + '' + color + 'position:absolute; top:' + y + 'px; height: ' + hb + 'px; left:' + x_start + 'px; width:' + Math.max(0, x_end - x_start) + 'px;' + (ev._text_style || "") + '">';
...
function y_scale(d) {
	...
	for (var i=0; i<this.y_unit.length; i++){
		...
		// section 1
		html += "<tr class='" + stats.tr_className + "' style='" + stats.style_height + "'><td class='" + stats.td_className + "' style='background-color: #BBB; " + stats.style_width + " height:" + (stats.height - 1) + "px;'>" + stats.td_content + "</td>";
