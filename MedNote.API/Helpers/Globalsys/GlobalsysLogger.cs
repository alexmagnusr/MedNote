...
{
    public sealed class GlobalsysLogger : ITraceWriter
    {
        ...
        private void Log(TraceRecord record)
        {
            ...
            if (record.Exception != null && !string.IsNullOrWhiteSpace(record.Exception.GetBaseException().Message))
            {
                ...
                {
                    var exception = record.Exception as CoreException;
                    if (exception != null)
                    {
                        message.Append("").Append("Error: " + exception.Message + Environment.NewLine);
                        message.Append("").Append("Error Code: " + "EXCEÇÃO DO SISTEMA" + Environment.NewLine);
                    }
                }
                else
            ...
            if (!record.Exception.IsEmpty())
            {
                Usuario usuario = UsuarioModel.Instancia.UsuarioLogado();

                LogErro logErro = new LogErro()
                {
                    StackTrace = record.TryGetValue(v => v.Exception.StackTrace),
                    Mensagem = record.Message = record.TryGetValue(v => v.Exception.Message),
                    Acao = message.ToString(),
                    Usuario = usuario,
                    DataErro = DateTime.Now
                };
                LogErroModel.Instancia.Cadastrar(logErro);
            }
